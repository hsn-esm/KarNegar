    ...
    Column(
        ...
        SectionCard {
            Row(Modifier.fillMaxWidth()) {
                ...
                Column(Modifier.weight(1f).padding(start = 10.dp)) {
                    ...
                    Holidays.dayOffTitle(today, state.holidayOverrides, state.remoteHolidays)
                        ?.let { title ->
                            Text(
                                text = title,
        ...
        SectionCard {
            ...
            MonthGrid(
                ...
                remoteHolidays = state.remoteHolidays,
        ...
        val holidays = buildList {
            for (day in 1..PersianDate.monthLength(state.visibleYear, state.visibleMonth)) {
                ...
                val title = Holidays.occasionTitle(d, state.remoteHolidays)
                if (title != null) add(Triple(d, title, Holidays.isDayOff(d, state.holidayOverrides, state.remoteHolidays)))
            }
        }
